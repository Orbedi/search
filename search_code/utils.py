...
# key= funcion que sirve como clave para ordenar la comparacion
# reverse= si es true, ordena la lista al reves.

#lambda -> Funcion que se define en un unica linea
...
class Coste(Queue):
    ...
    def extend(self, items):
        ...
        self.num += 1
        return self.array.sort(key=lambda x: x.path_cost, reverse=True) # reverse=True
...
class Heuristica(Queue):
    ...
    def extend(self, items):
        self.array.extend(items)
        self.num += 1
        return self.array.sort(key=lambda x: (x.path_cost+int(self.problem.h(x))), reverse=True) #reverse=True